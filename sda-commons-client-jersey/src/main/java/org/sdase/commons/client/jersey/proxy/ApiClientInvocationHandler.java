...
import org.sdase.commons.client.jersey.proxy.annotation.SuppressConnectTimeoutErrors;
import org.sdase.commons.client.jersey.proxy.annotation.SuppressHttpErrors;
import org.sdase.commons.client.jersey.proxy.annotation.SuppressProcessingErrors;
import org.sdase.commons.client.jersey.proxy.annotation.SuppressReadTimeoutErrors;
...
public class ApiClientInvocationHandler implements InvocationHandler {
  ...
  private boolean isHttpStatusSuppressed(Method invokedMethod, int httStatusCode) {
    SuppressHttpErrors suppressedHttpErrors =
        invokedMethod.getDeclaredAnnotation(SuppressHttpErrors.class);
  ...
  private boolean isProcessingErrorSuppressed(Method invokedMethod) {
    return invokedMethod.getDeclaredAnnotation(SuppressProcessingErrors.class) != null;
  }

  private boolean isConnectTimeoutErrorSuppressed(Method invokedMethod) {
    return invokedMethod.getDeclaredAnnotation(SuppressConnectTimeoutErrors.class) != null;
  }

  private boolean isReadTimeoutErrorSuppressed(Method invokedMethod) {
    return invokedMethod.getDeclaredAnnotation(SuppressReadTimeoutErrors.class) != null;
  }
}
